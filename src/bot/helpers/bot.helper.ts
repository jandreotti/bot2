...
): Promise<{ filePath: string; title: string; gifPlayback: boolean }> => {
  try {
    ...
    const mimeType =
      message.message[messageType].mimetype ||
    ...
    const title =
      message.message[messageType].title ||
      message.message['documentWithCaptionMessage']?.message?.documentMessage
        ?.title ||
      '';
    ...
    const extension =
      ...
      messageType === 'videoMessage'
        ? mimeType.split('/')[1]
        : message.message[messageType].title?.split('.').pop() ||
          message.message[
            'documentWithCaptionMessage'
