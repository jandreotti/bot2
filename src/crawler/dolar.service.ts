...
export class DolarService {
  ...
  async checkearDolarFetch(): Promise<DolarState> {
    try {
      ...
      let compraS = html
        .split('BluePromedio')[1]
        .split('colCompraVenta')[1]
        .split('data-order="$ ')[1]
        .split('"')[0]; //Blue
      let ventaS = html
        .split('BluePromedio')[1]
        .split('colCompraVenta')[2]
        .split('data-order="$ ')[1]
        .split('"')[0]; // Blue
      ...
      const change =
        this.estadoDolar.checkedOnce &&
        (this.estadoDolar.compra !== compra ||
          this.estadoDolar.venta !== venta);
